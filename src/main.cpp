...
const string strMessageMagic = "Motile Signed Message:\n";
		...
		if (MasternodePayments)
		{
			...
			if (IsProofOfStake() && pindex != NULL) {
				if (pindex->GetBlockHash() == hashPrevBlock) {
					...
					CMotilecoinAddress address2(address1);
